...
/**
 * Dashboard Component
 * The central command center of the LaundryLink application.
 * Synchronized with PredictionService for real-time profitability and cycle timers.
 */
const Dashboard = () => {
  ...
  /**
   * TELEMETRY SYNC
   * Uses concurrent fetching to retrieve shop-wide KPIs and individual machine states.
   * Prevents UI blocking by handling endpoint responses independently.
   */
  const loadDashboardData = useCallback(async (isRefresh = false) => {
    try {
      ...
      // Concurrent fetching for high-performance data hydration
      const [statsResult, machinesResult] = await Promise.allSettled([
        apiService.getDashboardStats(),
        apiService.getMachines(),
      ]);

      // Populate Global Stats (Revenue, Utilization, Forecasts)
      if (statsResult.status === 'fulfilled' && statsResult.value) {
        setStats(statsResult.value);
      }

      // Populate Machine-specific telemetry (Remaining Time, Profitability Rate)
      ...
      console.error("Critical Dashboard Sync Failure:", err.message);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
   ...
   * 1. Immediate initial load upon component mounting.
   * 2. Automated 60-second heartbeat to keep telemetry fresh without user interaction.
   */
  useEffect(() => {
    loadDashboardData();
  ...
  const handleBookingSuccess = () => {
    ...
    loadDashboardData(true); // Trigger immediate re-sync to show the new busy machine
  };
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen space-y-10 font-sans">
      ...
      {/* SECTION 1: SYSTEM IDENTITY & GLOBAL ACTIONS */}
      <div className="flex flex-col lg:flex-row justify-between items-start gap-6">
        <div>
          ...
          <p className="text-slate-500 text-sm font-bold mt-1 max-w-md">
            Monitoring real-time operational efficiency and predictive growth models.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 w-full lg:w-auto">
          <div className="hidden md:flex flex-col items-end bg-white px-6 py-3 rounded-[28px] border border-slate-200 shadow-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Current Date</span>
            </div>
      ...
      {/* SECTION 2: FINANCIAL & OPERATIONAL KPIs */}
        ...
        <div className="lg:col-span-2 bg-white p-10 rounded-[56px] border border-slate-100 shadow-sm relative overflow-hidden">
          ...
          <div className="flex justify-between items-start mb-10 relative z-10">
            <div>
              ...
              <h2 className="text-3xl font-black text-slate-900 tracking-tight">Income Forecast</h2>
              <p className="text-slate-400 text-xs font-bold mt-1">
                7-Day predictive modeling based on historical cycle frequency.
              </p>
            </div>
            ...
            {stats?.forecast_data?.length > 0 ? (
              ...
              <div className="h-full w-full flex flex-col items-center justify-center bg-slate-50/50 rounded-[40px] border-2 border-dashed border-slate-200">
                ...
                <p className="text-slate-400 font-black text-[10px] uppercase tracking-[0.2em]">
                  Generating prediction nodes...
                </p>
              </div>
            )}
          </div>

          {/* DYNAMIC BREAKDOWN: Sourced from machine metrics telemetry */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 border-t border-slate-50 pt-10 relative z-10">
            ...
            <BreakdownItem label="Weight Handled" value={`${stats?.total_weight || 0}kg`} />
          </div>
        </div>

        <div className="lg:col-span-1">
          {/* OptimizationTip: Displays the AI-generated insight from the backend Stat summary */}
          <OptimizationTip
            title={stats?.optimization?.title || "Profit Optimization"}
            message={stats?.optimization?.description || "Backend is monitoring electricity overhead for Dryers to optimize utility spend."}
            suggestion={stats?.optimization?.action_text || "Gathering Data"}
          />
        </div>
      </div>
      ...
      <div className="bg-white p-10 rounded-[56px] border border-slate-100 shadow-sm">
        <div className="flex justify-between items-center mb-10">
          <div>
            <div className="flex items-center gap-2 text-emerald-500 mb-1">
              <CheckCircle size={16} />
              <span className="text-[10px] font-black uppercase tracking-widest">Hardware Status</span>
            </div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">Machine Telemetry</h2>
            <p className="text-slate-400 text-xs font-bold">
              Real-time synchronization of hardware health and remaining service time.
            </p>
          </div>
          <button 
        ...
        {/* MachineGrid displays the individual cards with updated profitability rates */}
        <MachineGrid
          machines={machines}
...
// --- STATISTICAL BREAKDOWN SUB-COMPONENT ---
