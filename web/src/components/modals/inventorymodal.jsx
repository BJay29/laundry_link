...
const InventoryModal = ({ isOpen, onClose, item, onSave }) => {
  ...
  // Sync modal data when item prop changes
  useEffect(() => {
    if (item) {
      setFormData({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-[32px] p-8 w-full max-w-sm shadow-2xl border border-slate-100">
        ...
        <h3 className="text-xl font-black text-slate-900 tracking-tight mb-1">Update {item?.item_name}</h3>
        <p className="text-slate-500 text-sm font-medium mb-6">Modify stock levels and alerts.</p>

        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label className="block text-xs font-black text-slate-400 uppercase tracking-wider mb-2">Current Stock ({item?.unit})</label>
            <input 
              type="number"
